...
public class Roulette : MonoBehaviour
{
    ...
    private void Update()
    {
        if (_isPlacedFirst && !_isPlacedSecond)
        {
            line.SetPosition(0, firstPoint.transform.position);
            line.SetPosition(1, cameraVisual.transform.position);
        }
    }
    public void Checking()
    {
        if (action.GetStateDown(SteamVR_Input_Sources.Any))
        {
            if (!_isPlacedFirst)
            {
                firstPoint.transform.position = cameraVisual.transform.position;
                _isPlacedFirst = true;
            }
            else if (!_isPlacedSecond)
            {
                secondPoint.transform.position = cameraVisual.transform.position;
